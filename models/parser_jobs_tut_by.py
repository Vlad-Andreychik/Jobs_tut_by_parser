"""
Module is written to parse info from jobs.tut.by
"""
import re
...
class RabotaByParser:
    ...
    def get_list_of_vacancy_id(text):
        ...
        list_with_unique_vacancies = set(vacancies_list)
        vacancies_id_list = []
        for vacancy in list_with_unique_vacancies:
            pattern_id = '\d{8}'
            vacancy_id = re.findall(pattern_id, vacancy)
            vacancies_id_list.append(vacancy_id[0])
